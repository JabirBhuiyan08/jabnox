...
const Devices = () => {
    ...
    const {data: stats=[]} = useQuery({
        queryKey: ['devices'],
        queryFn: async () => {
            const res = await axiosSecure.get('/analytics/devices');
            return res.data;
        },
    })
    return (
        ...
        <ul className="divide-y divide-gray-200">   
            ...
            >
              <span className="font-medium text-gray-200">{row.device}</span>
              <span className="font-medium text-gray-200">Views: {row.users}</span>
            </li>
          ))}
        </ul>
      )}
